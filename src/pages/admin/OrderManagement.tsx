...
const OrderManagement: React.FC<OrderManagementProps> = ({ rolId }) => {
  ...
  const handleSearch = () => {
    if (searchOrderId) {
      navigate(`/admin/orders/${searchOrderId}`);
    }
  };
          ...
          {orderId ? (
            <>
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-xl font-semibold text-gray-800">Pedido #{orderId}</h2>
            ...
            <div className="text-center py-8">
              <p className="text-gray-600">
                Ingrese un ID de pedido para ver los detalles y actualizar su estado
              </p>
            </div>
          )}
        </div>
